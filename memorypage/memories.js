document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Cute Background Animation (Copied from homepage.js) ---
    const canvas = document.getElementById('cuteCanvas');
    if (!canvas) {
        console.error("Canvas element #cuteCanvas not found!");
        return;
    }
    const ctx = canvas.getContext('2d');
    let particlesArray = [];
    
    // Particle colors - ensure these match your desired theme for this page
    // These are from the previous homepage.js, adjust if needed for memories.html
    ...
    // Background gradient colors (ensure these are defined or use direct values)
    // These should match the 'brand-bg-start' and 'brand-bg-end' from your Tailwind config
    ...
    function setupCanvas() {
        ...
        // Apply gradient background to the canvas itself
    ...
    function animateParticles() {
        // Redraw canvas background each frame
        const gradient = ctx.createLinearGradient(0, 0, 0, canvas.height);
        gradient.addColorStop(0, brandBgStart);
        gradient.addColorStop(1, brandBgEnd);
        ctx.fillStyle = gradient;
