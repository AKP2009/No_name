document.addEventListener('DOMContentLoaded', () => {
    // --- DOM Elements ---
    const backButton = document.getElementById('backToMemoriesButton');

    // --- Back Button Logic ---
    if (backButton) {
        backButton.addEventListener('click', () => {
            // Navigate back to the main memories page
            window.location.href = '../../memorypage/memories.html';  // Adjusted path assuming it's in a 'memories' subfolder
        });
    }

    // --- Cute Background Animation (Same as other content pages) ---
    const canvas = document.getElementById('cuteCanvas');
    if (!canvas) {
        console.error("Canvas element #cuteCanvas not found for memory_special_day.js!");
        return;
    }
    const ctx = canvas.getContext('2d');
    if (!ctx) {
        console.error("Failed to get 2D context for memory_special_day.js!");
        return;
    }
    let particlesArray = [];
    
    const particleColors = [
        'rgba(240, 166, 202, 0.7)', 
        'rgba(190, 149, 196, 0.7)', 
        'rgba(224, 216, 245, 0.6)'  
    ];
